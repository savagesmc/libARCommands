...
#include <config.h>
#include <glib.h>
#include <epan/dissectors/pidl/idl_types.h>
#include <epan/packet.h>
#include <epan/proto.h>
#include <epan/expert.h>
#include <epan/range.h>
...
#ifdef SKIP_COMMAND_FIELDS
   DEBUG("skipping add_command_fields\n");
   return;
#endif

   DEBUG("add_command_fields\n");
...
#ifdef SKIP_FRAME_FIELDS
   DEBUG("skipping add_frame_fields\n");
   return;
#endif

   DEBUG("add_frame_fields\n");
...
#ifdef SKIP_STREAM_FIELDS
   DEBUG("skipping add_stream_fields\n");
   return;
#endif
   DEBUG("add_stream_fields\n");
...
#ifdef SKIP_PINGPONG_FIELDS
   DEBUG("skipping add_pingpong_fields\n");
   return;
#endif
   DEBUG("add_pingpong_fields\n");
...
#ifdef SKIP_ACK_FIELDS
   DEBUG("skipping add_ack_fields\n");
   return;
#endif
   DEBUG("add_ack_fields\n");
...
void proto_register_arsdk(void)
{
   ...
   DEBUG("proto_register_arsdk\n");

   wmem_pool = wmem_allocator_new(WMEM_ALLOCATOR_SIMPLE);
   ...
   DEBUG("proto_register_field_array\n");
   proto_register_field_array(proto_arsdk, hf_arsdk_info, hf_arsdk_nitems);
   DEBUG("proto_register_field_array - complete\n");

   DEBUG("proto_register_subtree_array\n");
   proto_register_subtree_array(ett, array_length(ett));
   DEBUG("proto_register_subtree_array - complete\n");
...
proto_reg_handoff_arsdk(void)
{
   ...
   if (!initialized) {
       ...
       */
      arsdk_handle = create_dissector_handle(dissect_arsdk, proto_arsdk);
      initialized = TRUE;
   ...
   arsdk_port_range = range_copy(wmem_pool, global_arsdk_port_range);
